...
This source/design assessment does not establish actual Andrix DRM playback or
service eligibility. The accepted decision is that **Andrix does not need Silvervine**:
no bundle, port or optional-package integration is planned. The source comparison is retained as rationale. Retaining genuine
...
Browser mode and service eligibility can affect playback independently of whether a
DRM component is installed. A mobile-web rejection does not establish a missing CDM,
an L1/L3 failure or the exact enforcement mechanism. Do not add a desktop CDM installer
to solve an unproven Android component problem. Individual device/session observations
are retained privately and are not Andrix compatibility guarantees.
...
an unproven missing-component problem. The source distinction explains the scope
decision; it does not substitute for actual Andrix playback or network qualification.
