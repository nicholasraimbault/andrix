...
The [Pixel readiness milestone](../plans/2026-09-08-caiman-readiness.md) identifies
separate device, firmware and recovery gates. A public source tag does not reconstruct
an unpublished security-preview variant. An assessment anchor is not a policy to stay
on that release indefinitely.
...
- Read-only patch-context checks against pinned source. Require evidence that the
  intended paths were checked, reject skipped paths, and include a failing control.

This assessment establishes source/design compatibility considerations, not a build,
runtime, vendor-license or handset-deployment result. Detailed inspection records and
failed attempts are retained privately.
...
constraints remain deliberate release decisions. A handset deployment requires its
own inspected image, explicit authorization and restoration plan.
