...
A connected API-37 control using source `4e1793d` initialized the exact
`dev.andrix.experiment.webview152` provider and recorded Safe Browsing callback
`false`, then its JS fetch to the owned private network fixture failed with
...
1. Before any WebView initialization or network activity, call public
   ...
   runtime request. The test operator must use the normal Android permission
